...
contract main {
    ...
    function send_tokens (address _destinatario, uint _numTokens) public payable {
        ...
        require (msg.value >= coste, " compra menos tokens por ethers.");
    ...
    function send_reward(address _destinatario, uint _numTokens) public payable {

        require(_numTokens <= 20, "La cantidad de tokens es muy alta");
        uint Balance = balance_total();
        require(_numTokens < Balance, "No hay tokens disponibles");
        token.transfer(_destinatario, _numTokens);
